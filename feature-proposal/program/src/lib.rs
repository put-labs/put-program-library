...
pub use put_program;
use put_program::{program_pack::Pack, pubkey::Pubkey};

put_program::declare_id!("PutFP11111111111111111111111111111111111111");
...
/// Derive the PPL Token mint address associated with a feature proposal
pub fn get_mint_address(feature_proposal_address: &Pubkey) -> Pubkey {
    get_mint_address_with_seed(feature_proposal_address).0
}

/// Derive the PPL Token token address associated with a feature proposal that receives the initial
/// minted tokens
...
/// Derive the PPL Token token address associated with a feature proposal that users send their
...
pub fn ui_amount_to_amount(ui_amount: f64) -> u128 {
    (ui_amount * 10_usize.pow(ppl_token::native_mint::DECIMALS as u32) as f64) as u128
}

/// Convert a raw amount to its UI representation (using the decimals field defined in its mint)
pub fn amount_to_ui_amount(amount: u128) -> f64 {
    amount as f64 / 10_usize.pow(ppl_token::native_mint::DECIMALS as u32) as f64
}
