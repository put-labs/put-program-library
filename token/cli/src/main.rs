...
use console::Emoji;
use ppl_associated_token_account::*;
use ppl_token::{
    self,
    instruction::*,
    native_mint,
    state::{Account, Mint, MintMeta, Multisig},
};
use put_account_decoder::{
...
use put_cli_output::{display::println_name_value, return_signers, OutputFormat};
use put_client::{
...
type CommandResult = Result<Option<(u128, Vec<Vec<Instruction>>)>, Error>;
...
fn check_fee_payer_balance(config: &Config, required_balance: u128) -> Result<(), Error> {
...
fn command_create_token(
    ...
    let minimum_balance_for_rent_exemption_mint_info = if !config.sign_only {
        config
            .rpc_client
            .get_minimum_balance_for_rent_exemption(MintMeta::LEN)?
    } else {
        0
    };
    ...
    let (mint_info, _) =
        Pubkey::find_program_address(&[b"MintMeta", &token.to_bytes()], &ppl_token::id());

    println!("Creating token {}", token);
    println!("Creating token meta {}", mint_info);
    ...
    let mut instructions = vec![
        ...
        create_mint_meta_account(&ppl_token::id(), &config.fee_payer, &token, &mint_info)?,
        initialize_mint(
            &ppl_token::id(),
            &token,
            &authority,
    ...
    if let Some(text) = memo {
        instructions.push(ppl_memo::build_memo(text.as_bytes(), &[&config.fee_payer]));
    }
    Ok(Some((
        minimum_balance_for_rent_exemption + minimum_balance_for_rent_exemption_mint_info,
        vec![instructions],
    )))
}
    ...
    let (account, system_account_ok, instructions) = if let Some(account) = maybe_account {
        ...
        (
            ...
            vec![
                ...
                initialize_account(&ppl_token::id(), &account, &token, &owner)?,
            ],
        )
    } else {
...
fn command_authorize(
    ...
    let auth_str = match authority_type {
        ...
        AuthorityType::UpdateMeta => "update mint meta authority",
    };
        ...
        if let Ok(mint) = Mint::unpack(&target_account.data) {
            match authority_type {
                AuthorityType::AccountOwner | AuthorityType::CloseAccount => Err(format!(
                    "Authority type `{}` not supported for PPL Token mints",
                    auth_str
                )),
                AuthorityType::MintTokens => Ok(mint.mint_authority),
                AuthorityType::FreezeAccount => Ok(mint.freeze_authority),
                AuthorityType::UpdateMeta => {

                    if account == ppl_token::native_mint::id() {
                        Err("Unsupported on native mint account ".to_string())
                    }else{
                        let (mint_meta, _) =
                        Pubkey::find_program_address(&[b"MintMeta", &account.to_bytes()], &ppl_token::id());
                        let account_data = config.rpc_client.get_account_data(&mint_meta)?;
                        let meta_data = MintMeta::unpack_from_slice(account_data.as_slice())?;
                        Ok(meta_data.authority)
                    }
                },
            }
            ...
            match authority_type {
                AuthorityType::MintTokens | AuthorityType::FreezeAccount | AuthorityType::UpdateMeta => Err(format!(
                    "Authority type `{}` not supported for PPL Token accounts",
                    auth_str
                )),
...
fn validate_mint_meta_auth(config: &Config, token_meta: &Pubkey, auth: &Pubkey) -> Result<(), Error> {
    let mint_meta = config.rpc_client.get_account(&token_meta);
    if mint_meta.is_err() {
        return Err(format!("Invalid mint_meta account {:?}", token_meta).into());
    }
    let meta_account = MintMeta::unpack(&mint_meta.unwrap().data)?;
    if let COption::Some(meta_auth) = meta_account.authority {
        if meta_auth != *auth {
            return Err(format!(
                "Error auth account for  {:?} , except {:?}, supply {:?}.",
                token_meta, meta_auth, auth
            )
            .into());
        }
    } else {
        return Err(format!("Invalid auth account {:?}", token_meta).into());
    }
    Ok(())
}
...
fn command_transfer(
    ...
    let maybe_transfer_balance =
        ui_amount.map(|ui_amount| {
            let (inte,frac) = parse_amount(ui_amount.as_str());
            ppl_token::ui_amount_to_amount(inte,frac, decimals)
        });
    let transfer_balance = if !config.sign_only {
        ...
        let sender_balance = sender_token_amount.amount.parse::<u128>().map_err(|err| {
            format!(
        ...
        println!(
            "Transfer {} tokens\n  Sender: {}\n  Recipient: {}",
            ppl_token::amount_to_ui_amount(transfer_balance, decimals),
            sender,
            recipient
        );
    ...
    let recipient_is_token_account = if !config.sign_only {
        let recipient_account_info = config
            ...
            .value
            .map(|account| account.owner == ppl_token::id() && account.data.len() == Account::LEN);
        ...
        let needs_funding = if !config.sign_only {
                ...
                .value
            {
                if recipient_token_account_data.owner == system_program::id() {
                    true
                } else if recipient_token_account_data.owner == ppl_token::id() {
                    false
                } else {
                    return Err(
            ...
            if fund_recipient {
                if !config.sign_only {
                    minimum_balance_for_rent_exemption += config
                        ...
                        .get_minimum_balance_for_rent_exemption(Account::LEN)?;
                    println!(
                        "  Funding recipient: {} ({} PUT)",
                        recipient_token_account,
                        lamports_to_put(minimum_balance_for_rent_exemption)
                    );
                }
...
fn command_burn(
    ...
    let (inte,frac) = parse_amount(ui_amount.as_str());
    let amount = ppl_token::ui_amount_to_amount(inte,frac, decimals);
...
fn command_mint(
    ...
    if amount == 0 {
        return Err(format!("Minting Token amount too small ,  token decimals is {}", decimals).into());
    }
...
fn command_wrap(
    ...
    wrapped_put_account: Option<Pubkey>,
) -> CommandResult {
    let lamports = put_to_lamports(&put);

    if lamports == 0 {
       return Err(format!("Wrap PUT amount too small ,  PUT decimals is 9.").into());
    }

    let instructions = if let Some(wrapped_put_account) = wrapped_put_account {
        println!("Wrapping {} PUT into {}", put, wrapped_put_account);
        vec![
            system_instruction::create_account(
                &wallet_address,
                &wrapped_put_account,
        ...
        println!("Wrapping {} PUT into {}", put, account);
        vec![
    ...
    if !config.sign_only {
        ...
        if lamports == 0 {
            if use_associated_account {
                return Err("No wrapped PUT in associated account; did you mean to specify an auxiliary address?".to_string().into());
            } else {
                return Err(format!("No wrapped PUT in {}", address).into());
            }
        }
        println!("  Amount: {} PUT", lamports_to_put(lamports),);
    }
...
fn command_accounts(config: &Config, token: Option<Pubkey>, owner: Pubkey) -> CommandResult {
    ...
    let accounts = config.rpc_client.get_token_accounts_by_owner(
        ...
        match token {
            ...
            None => TokenAccountsFilter::ProgramId(ppl_token::id()),
        },
    )?;
    ...
    if gc_alert {
        println!();
        println!("* Please run `ppl-token gc` to clean up Aux accounts");
    }
    Ok(None)
}
...
fn command_gc(config: &Config, owner: Pubkey) -> CommandResult {
    ...
    let accounts = config
        .rpc_client
        .get_token_accounts_by_owner(&owner, TokenAccountsFilter::ProgramId(ppl_token::id()))?;
    if accounts.is_empty() {
    ...
    for keyed_account in accounts {
        if let UiAccountData::Json(parsed_account) = keyed_account.account.data {
            if parsed_account.program == "ppl-token" {
    ...
    for (token, accounts) in accounts_by_token.into_iter() {
        ...
        let total_balance: u128 = accounts.values().map(|account| account.0).sum();
...
fn command_sync_native(native_account_address: Pubkey) -> CommandResult {
    let instructions = vec![sync_native(&ppl_token::id(), &native_account_address)?];
    Ok(Some((0, vec![instructions])))
}

fn command_token_info(config: &Config, address: Pubkey) -> CommandResult {
    let (mut mint_info, _) =
        Pubkey::find_program_address(&[b"MintMeta", &address.to_bytes()], &ppl_token::id());

    if address == ppl_token::native_mint::id() {
        mint_info = ppl_token::native_mint_info::id();
    }

    let account_data = config.rpc_client.get_account_data(&mint_info)?;

    let token_data = MintMeta::unpack_from_slice(account_data.as_slice())?;

    println_name_value("Token:", address.to_string().as_str());
    let auth = match token_data.authority {
        COption::Some(key) => key.to_string(),
        COption::None => "None".to_string(),
    };
    println_name_value("Token Meta:", mint_info.to_string().as_str());
    println_name_value("Token Meta Authority:",auth.to_string().as_str());
    println_name_value("Token Symbol:", token_data.symbol.as_str());
    println_name_value("Token Name:", token_data.name.as_str());
    println_name_value("Token Icon:", token_data.icon.as_str());

    Ok(None)
}

fn command_update_token_info(
    config: &Config,
    token_meta: Pubkey,
    meta_auth: Pubkey,
    symbol: Option<String>,
    ...
    if symbol == None && name == None && icon == None {
        Ok(None)
    } else {
        validate_mint_meta_auth(config, &token_meta, &meta_auth)?;
        let mut instructions = vec![];
        if symbol != None {
            let symbol = symbol.unwrap();
            let idx = update_token_symbol(
                &ppl_token::id(),
                &token_meta,
                &meta_auth,
                &config.multisigner_pubkeys,
            ...
            .unwrap();
            let mut ins_symbol = vec![];
            ins_symbol.push(idx);
            instructions.push(ins_symbol)
        }

        if name != None {
            let name = name.unwrap();
            let idx = update_token_name(
                &ppl_token::id(),
                &token_meta,
                &meta_auth,
                &config.multisigner_pubkeys,
            ...
            .unwrap();
            let mut ins_name = vec![];
            ins_name.push(idx);
            instructions.push(ins_name)
        }

        if icon != None {
            let icon = icon.unwrap();
            let idx = update_token_icon(
                &ppl_token::id(),
                &token_meta,
                &meta_auth,
                &config.multisigner_pubkeys,
            ...
            .unwrap();
            let mut ins_icon = vec![];
            ins_icon.push(idx);
            instructions.push(ins_icon)
        }
...
fn main() {
    ...
    let app_matches = App::new(crate_name!())
        ...
        .subcommand(SubCommand::with_name("create-token").about("Create a new token")
                ...
                .arg(
                    Arg::with_name("symbol")
                        ...
                        .takes_value(true)
                        .help(
                            "Specify a symbol for the token is created. max len is 8 bytes."
                        ),
                ).arg(
                    Arg::with_name("name")
                        .long("name")
                        .takes_value(true)
                        .help(
                            "Specify a name for the token is created. max len is 32 bytes."
                        ),
                )
                .arg(
                    Arg::with_name("icon")
                        ...
                        .takes_value(true)
                        .help(
                            "Specify a icon url address for the token is created. max len is 128 bytes."
                        ),
                )
                .arg(
                    Arg::with_name("memo")
            ...
            SubCommand::with_name("authorize")
                ...
                .arg(
                    Arg::with_name("authority_type")
                        ...
                        .takes_value(true)
                        .possible_values(&["mint", "freeze", "owner", "close", "meta"])
                        .index(2)
                        .required(true)
                        .help("The new authority type. \
                            Token mints support `mint` , `freeze` and `meta` authorities;\
        ...
        .subcommand(
            SubCommand::with_name("wrap")
                .about("Wrap native PUT in a PUT token account")
                .arg(
                    Arg::with_name("amount")
                        .validator(is_amount)
                        .value_name("AMOUNT")
                ...
                .arg(
                    Arg::with_name("wallet_keypair")
                        ...
                        .takes_value(true)
                        .help(
                            "Specify the keypair for the wallet which will have its native PUT wrapped. \
                             This wallet will be assigned as the owner of the wrapped PUT token account. \
                             This may be a keypair file or the ASK keyword. \
                             Defaults to the client keypair."
                        ),
                )
                .arg(
                    Arg::with_name("create_aux_account")
                        ...
                        .help("Wrap PUT in an auxillary account instead of associated token account"),
                )
                .nonce_args(true)
                .offline_args(),
        )
        .subcommand(
            SubCommand::with_name("unwrap")
                .about("Unwrap a PUT token account")
                .arg(
                    Arg::with_name("address")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ACCOUNT_ADDRESS")
                ...
                .arg(
                    Arg::with_name("wallet_keypair")
                        ...
                        .takes_value(true)
                        .help(
                            "Specify the keypair for the wallet which owns the wrapped PUT. \
                             This wallet will receive the unwrapped PUT. \
                             This may be a keypair file or the ASK keyword. \
                             Defaults to the client keypair."
                        ),
                )
            ...
            SubCommand::with_name("close")
                ...
                .arg(
                    Arg::with_name("recipient")
                        ...
                        .takes_value(true)
                        .help("The address of the account to receive remaining PUT [default: --owner]"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("account-info")
                .about("Query details of an PPL Token account by address")
                .arg(
                    Arg::with_name("token")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ADDRESS")
        ...
        .subcommand(
            SubCommand::with_name("multisig-info")
                .about("Query details about and PPL Token multisig account by address")
                .arg(
                    Arg::with_name("address")
                    .validator(is_valid_pubkey)
                    ...
                    .index(1)
                    .required(true)
                    .help("The address of the PPL Token multisig account to query"),
                ),
        )
        .subcommand(
        ...
        .subcommand(
            SubCommand::with_name("sync-native")
                .about("Sync a native PUT token account to its underlying lamports")
                .arg(
                    owner_address_arg()
                        .index(1)
                        .conflicts_with("address")
        ...
        .subcommand(
            SubCommand::with_name("token-info")
                .about("Query details of an PPL Token")
                .arg(
                    Arg::with_name("token")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ADDRESS")
        ...
        .subcommand(
            SubCommand::with_name("update-token-info")
                .about("Update info of an PPL Token")
                .arg(
                    Arg::with_name("token")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ADDRESS")
                ...
                .arg(
                    ...
                    .takes_value(true)
                    .help(
                        "Specify the mint meta data update authority address. \
                         Defaults to the client keypair address."
                    ),
                )
                .arg(
                    Arg::with_name("symbol")
                        .long("symbol")
                        .takes_value(true)
                        .help("Specify the symbol for token to update"),
                ).arg(
                    Arg::with_name("name")
                        .long("name")
                        .takes_value(true)
                        .help("Specify the name for token to update"),
                )
                .arg(
                    Arg::with_name("icon")
                        .long("icon")
                        .takes_value(true)
                        .help("Specify the icon for token to update"),
                ),
        )
        .get_matches();
    ...
    let _ = match (sub_command, sub_matches) {
        ("create-token", Some(arg_matches)) => {
            ...
            let symbol = value_t!(arg_matches, "symbol", String).ok();
            let name = value_t!(arg_matches, "name", String).ok();
            let icon = value_t!(arg_matches, "icon", String).ok();
        ...
        ("transfer", Some(arg_matches)) => {
            ...
            let amount = match matches.value_of("amount").unwrap() {
                "ALL" => None,
                amount => Some(amount.parse::<String>().unwrap()),
            };
        ...
        ("burn", Some(arg_matches)) => {
            ...
            let amount = value_t_or_exit!(arg_matches, "amount", String);
            let mint_address =
        ...
        ("token-info", Some(arg_matches)) => {
            let address = pubkey_of_signer(arg_matches, "token", &mut wallet_manager)
                .unwrap()
                .unwrap();
            command_token_info(&config, address)
        }
        ("update-token-info", Some(arg_matches)) => {
            let token = pubkey_of_signer(arg_matches, "token", &mut wallet_manager)
                .unwrap()
                .unwrap();

            let (authority_signer, meta_authority) =
                config.signer_or_default(arg_matches, "meta_authority", &mut wallet_manager);
            bulk_signers.push(authority_signer);
